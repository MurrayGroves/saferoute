...
function getTransitOperatorComparatorValue(route, transitOperators) {
  ...
  var transitOperator = getTransitOperatorFromOtpRoute(route, transitOperators); // if transit operator not found, return infinity
...
export function makeStringValueComparator(objGetterFn) {
  return function (a, b) {
    var _getSortValues2 = getSortValues(objGetterFn, a, b),
        ...
        bVal = _getSortValues2.bVal; // both a and b are uncomparable strings, return equivalent value
...
function makeMultiCriteriaSort() {
  ...
  return function (a, b) {
    for (var i = 0; i < criteria.length; i++) {
      var curCriteriaComparatorValue = criteria[i](a, b); // if the comparison objects are not equivalent, return the value obtained
