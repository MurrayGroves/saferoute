...
export function getLegCost(leg, category, container) {
  var _relevantFareProducts, _transferFareProduct$;

  if (!leg.fareProducts) return {
    price: undefined
  };
  var relevantFareProducts = leg.fareProducts.filter(function (fp) {
    return fp.category.name === category && fp.container.name === container;
  });
  var totalCost = (_relevantFareProducts = relevantFareProducts.find(function (fp) {
    return fp.name === "rideCost";
  })) === null || _relevantFareProducts === void 0 ? void 0 : _relevantFareProducts.amount;
  var transferFareProduct = relevantFareProducts.find(function (fp) {
    return fp.name === "transfer";
  });
  return {
    price: totalCost,
    transferAmount: transferFareProduct === null || transferFareProduct === void 0 ? void 0 : (_transferFareProduct$ = transferFareProduct.amount) === null || _transferFareProduct$ === void 0 ? void 0 : _transferFareProduct$.cents
  };
}
/**
...
export function getItineraryCost(legs, category, container) {
  return legs.filter(function (leg) {
    return !!leg.fareProducts;
  }).map(function (leg) {
    return getLegCost(leg, category, container).price;
  }).reduce(function (prev, cur) {
    var _prev$currency;

    return {
      cents: prev.cents + (cur === null || cur === void 0 ? void 0 : cur.cents) || 0,
